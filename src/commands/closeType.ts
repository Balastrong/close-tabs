...
export const closeType = async (uri?: vscode.Uri) => {
    try {
        ...
        if (extensionsToClose.length === 0 && uri) {
            const fileExtension = uri.fsPath.split('.').pop();
            if (fileExtension) {
                extensionsToClose = [fileExtension];
                console.log(`No extensions specified in settings. Using the extension of the right-clicked file: ${fileExtension}`);
            }
        }

        // Normalize extensions to remove leading dots
        const normalizedExtensionsToClose = extensionsToClose.map(ext => ext.startsWith('.') ? ext.slice(1) : ext);
        const finalExtensionsToClose = caseSensitive ? normalizedExtensionsToClose : normalizedExtensionsToClose.map(ext => ext.toLowerCase());
        ...
        vscode.window.tabGroups.all
            ...
            .forEach((tab) => {
                ...
                if (fileExtension) {
                    const normalizedExtension = caseSensitive ? fileExtension : fileExtension.toLowerCase();
                    ...
                    if (finalExtensionsToClose.includes(normalizedExtension)) {
                        ...
                        console.log(`File extension is not in the list to close: ${fileName}`);
                    }
                }
            });
